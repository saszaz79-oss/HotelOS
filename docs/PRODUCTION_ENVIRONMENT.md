...
**IPv4-only hosts (Vercel serverless included) cannot use Supabase's direct connection string**: `db.<ref>.supabase.co` publishes only an AAAA (IPv6) record on projects without the IPv4 add-on — confirmed by DNS lookup in production — so runtime connections from Vercel fail with Prisma `P1001: Can't reach database server`. Use the **session pooler** URL instead for the app's runtime `DATABASE_URL` (`postgres://postgres.<ref>:<password>@aws-0-<region>.pooler.supabase.com:5432/postgres`, port 5432 = session mode, full Postgres semantics); the same `DATABASE_CA_CERT` verification applies. GitHub Actions migration/seed can keep whichever form works there.
